...
2. Run the copy, writing the final document to the path the user asked for, or
   to the current working directory when the user gave only a filename or no
   output path:

   ```sh
   kowork-node <task-temp-dir>/create_docx.cjs <current-working-directory>/output.docx
   ```

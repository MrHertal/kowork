...
| `pnpm preview:worker` | Build, then preview the full Worker deployment incl. `/download` |
...
To preview the complete deployment locally, including `/download`, run `pnpm --filter @kowork/web preview:worker`. The regular `pnpm dev` command only starts Astro's static-site development server.

#!/usr/bin/env python3
"""Repack a directory of OOXML parts back into a .docx package.
...
package without it. Editor junk such as .DS_Store is skipped.

Pass --cleanup to delete the working directory after a successful pack, so no
unpacked XML is left beside the user's document. As a safety net it only deletes
a directory that lives under a temp dir (where the edit flow should unpack); a
work dir anywhere else is left in place with a note.

Usage:
    kowork-python pack.py <source_dir> <output.docx> [--cleanup]
"""
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Pack an unpacked OOXML tree into a .docx.")

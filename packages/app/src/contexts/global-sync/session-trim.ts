// @opencode-ref: opencode/packages/app/src/context/global-sync/session-trim.ts
...
export function takeRecentSessions(
  ...
  for (const session of sessions) {
    ...
    const index = selected.findIndex(
      (x) => compareSessionRecent(session, x) < 0,
    );
...
export function trimSessions(
  ...
  const recent = takeRecentSessions(
    roots.slice(limit),
    SESSION_RECENT_LIMIT,
    cutoff,
  );
  const recentIds = new Set(recent.map((s) => s.id));
  const protectedRoots = roots.filter(
    (s) => protect.has(s.id) && !baseIds.has(s.id) && !recentIds.has(s.id),
  );
  const keepRoots = [...base, ...recent, ...protectedRoots];
  const keepRootIds = new Set(keepRoots.map((s) => s.id));
  const keepChildren = children.filter((s) => {
    if (s.parentID && keepRootIds.has(s.parentID)) return true;
    if (protect.has(s.id)) return true;

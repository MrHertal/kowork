...
macOS releases are signed (Developer ID Application) and notarized (App Store Connect API key), enabled entirely by repo secrets: `release.yml` imports the cert from `CSC_LINK` (base64 .p12) + `CSC_KEY_PASSWORD` into a temporary keychain and writes the `.p8` contents from `APPLE_API_KEY` to a temp file; `electron-builder.config.ts` turns on signing and `notarize: true` automatically once `CSC_LINK` + `APPLE_API_KEY`/`APPLE_API_KEY_ID`/`APPLE_API_ISSUER` are set. A missing secret fails the cert-import step loudly — never ship unsigned after a signed release: Squirrel.Mac rejects any update that doesn't satisfy the installed app's designated requirement, so an unsigned build breaks auto-update (this is also why unsigned v0.1.0 installs cannot auto-update to v0.1.1; those users reinstall manually once). The `afterSign` hook fails the build if any bundled runtime-pack Mach-O (embedded Python) is unsigned or not hardened. Workflows SHA-pin their actions; Dependabot (`.github/dependabot.yml`) opens weekly bump PRs.

...
`check.yml` runs typecheck, lint, and tests on PRs and `main`. `release.yml` is manual (Actions → release → Run workflow, semver input): it tags the current commit `v<version>`, builds the macOS arm64 installer on `macos-26`, and uploads a **draft** GitHub Release to publish manually. The tag points at an existing commit because `main` requires PRs; the version is applied at build time, then a `release v<version>` PR bumping `packages/electron/package.json` opens to merge afterwards. No x64 build: free Intel runners are retired (`macos-26-intel` is paid); the runtime pack builds host-arch only, so x64 needs that runner or a cross-build mode in `build-runtime.ts`. electron-builder may create a duplicate draft release carrying only `*.blockmap` files — fold its assets into the main draft and delete the empty one.

macOS releases are signed (Developer ID Application) and notarized, driven entirely by repo secrets: `release.yml` imports the `CSC_LINK`/`CSC_KEY_PASSWORD` cert and writes the `APPLE_API_KEY` .p8 to a temp file; `electron-builder.config.ts` enables signing and `notarize: true` automatically (with `APPLE_API_KEY_ID`/`APPLE_API_ISSUER`), and its `afterSign` hook fails the build if any runtime-pack Mach-O is unsigned or not hardened. Never ship unsigned after a signed release: Squirrel.Mac rejects updates that don't satisfy the installed app's designated requirement, so unsigned v0.1.0 installs can't auto-update and must reinstall manually once. Actions are SHA-pinned; Dependabot opens weekly bump PRs.

macOS icons: `mac.icon` points at `resources/icons/icon.icon` (Icon Composer source under `icons/<channel>/`, copied by `copy-icons.ts`), compiled via `actool` — macOS packaging requires Xcode 26+ (`release.yml` selects it), and Xcode 26's actool crashes on macOS 15 hosts (framework skew), hence the `macos-26` runner. `setDockIcon()` only affects dev; packaged builds use the bundle icon.

// @opencode-ref: opencode/packages/session-ui/src/components/part-default-open.test.ts
...
describe("partDefaultOpen", () => {
  test("keeps edited files expanded when enabled", () => {
    expect(
      partDefaultOpen(
        tool("edit", { filediff: { additions: 1, deletions: 1 } }),
        false,
        true,
      ),
    ).toBe(true);
  ...
  test("collapses deletion-only edits when enabled", () => {
    expect(
      partDefaultOpen(
        tool("edit", { filediff: { additions: 0, deletions: 1_200 } }),
        false,
        true,
      ),
    ).toBe(false);

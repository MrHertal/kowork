...
function probe(shell: string, mode: "-il" | "-l"): Probe {
  ...
  if (err) {
    ...
    console.log(
      `[server] Shell env probe failed for ${shell} ${mode}: ${err.message}`,
    );
  ...
  if (out.status !== 0) {
    console.log(
      `[server] Shell env probe exited with non-zero status for ${shell} ${mode}`,
    );
    return { type: "Unavailable" };
  }
  ...
  if (Object.keys(env).length === 0) {
    console.log(
      `[server] Shell env probe returned empty env for ${shell} ${mode}`,
    );
    return { type: "Unavailable" };
  }

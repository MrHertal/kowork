// @opencode-ref: opencode/packages/app/src/utils/diffs.test.ts
...
describe("diffs", () => {
  ...
  test("drops invalid entries", () => {
    ...
    ).toEqual([item, { file: "src/bad.ts", additions: 1, deletions: 1 }]);
  });

  test("keeps entries with an undefined patch", () => {
    const unpatchable = {
      file: "src/binary.png",
      patch: undefined,
      additions: 0,
    ...
    expect(diffs([unpatchable])).toEqual([unpatchable]);
  });
});

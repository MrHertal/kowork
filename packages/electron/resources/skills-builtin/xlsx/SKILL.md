...
2. Run the copy, writing the final workbook to the path the user asked for, or
   to the current working directory when the user gave only a filename or no
   output path:

   ```sh
   kowork-python <task-temp-dir>/create_xlsx.py <current-working-directory>/out.xlsx
   ```

...
function retryable(error: unknown, signal?: AbortSignal) {
  ...
  return /network|fetch|econnreset|econnrefused|enotfound|timedout/i.test(
    error.message,
  );
}
...
export async function checkServerHealth(
  ...
  const timeout = opts?.signal
    ? undefined
    : timeoutSignal(opts?.timeoutMs ?? defaultTimeoutMs);
  ...
  const attempt = (count: number): Promise<ServerHealth> =>
    createSdkForServer({ server, fetch, signal })
      .global.health()
      .then((x) =>
        x.error
          ? next(count, x.error)
...
export function useCheckServerHealth() {
  ...
  return (http: ServerConnection.HttpBase) => {
    ...
    const now = Date.now();
    if (hit && hit.fetch === fetcher && (!hit.done || now - hit.at < cacheMs))
      return hit.promise;
